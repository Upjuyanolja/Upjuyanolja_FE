...
describe('쿠폰 데이터', () => {
  ...
  /* 수정 */
  test('쿠폰 아이템 일일 제한 수량 수정 후 저장버튼 활성화되는지 확인한다.', () => {
    const { container } = render(
      <QueryClientProvider client={queryClient}>
        <BrowserRouter>
          <Coupon />
  ...
  test('쿠폰 아이템 일일 제한 수량에 문자 입력 시 block 되는지 확인한다', async () => {
    const { container } = render(
      <QueryClientProvider client={queryClient}>
        <BrowserRouter>
          <Coupon />
    ...
    fireEvent.change(inputBox, { target: { value: '23aaaa' } });
